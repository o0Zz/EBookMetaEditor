...
public sealed class ContainerTests
{
    ...
    /// <summary>
    /// The reason <c>ZipCentralDirectory</c> exists. <c>ZipArchiveEntry</c> does
    /// not expose the compression method, and an EPUB whose <c>mimetype</c> gets
    /// deflated on save is rejected by readers — so the method has to be read
    /// from the archive structure, not guessed from sizes.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// A stored entry whose content happens not to shrink would look identical
    /// to a deflated one under a <c>CompressedLength == Length</c> heuristic.
    /// This pins that the real method is read instead.
    /// </summary>
    [Fact]
    public void Stored_and_deflated_entries_are_distinguished_when_sizes_match()
    {
        using var temp = new TempDir();

        // Random-ish bytes: deflate cannot compress them, so compressed and
        // uncompressed lengths land close together for both entries.
        ...
        ContainerEntry stored = container.Entries.Single(e => e.Name == "OEBPS/stored.bin");
        ContainerEntry deflated = container.Entries.Single(e => e.Name == "OEBPS/deflated.bin");

        Assert.Equal(ZipCompressionMethods.Stored, stored.CompressionMethod);
        Assert.Equal(ZipCompressionMethods.Deflate, deflated.CompressionMethod);
    }

    /// <summary>
    /// Invariant 6: open a file, save without editing, get identical bytes.
    /// </summary>
    [Fact]
    ...
    public void Entry_content_round_trips_exactly()
    {
        ...
        Assert.Equal("application/epub+zip", System.Text.Encoding.UTF8.GetString(buffer.ToArray()));
    }

    [Fact]
    public void Truncated_archive_is_refused_rather_than_half_read()
    {
        using var temp = new TempDir();
        byte[] full = new EpubBuilder().Build();
        string path = temp.File("truncated.epub");
        ...
        Assert.Throws<BookFormatException>(() => ZipContainer.Open(path));
    }

    [Fact]
    public void Not_a_zip_is_refused()
    {
        using var temp = new TempDir();
        string path = temp.File("nonsense.epub");
        File.WriteAllBytes(path, System.Text.Encoding.UTF8.GetBytes(new string('x', 400)));
